...
const SingleBlog = () => {
    ...
    const editBlog = () => {
        axios.get(`http://localhost:4000/edit/${id}`).then((data) => {
            console.log(data.data);
            history.push('/editBlog', { data: data.data })
        }).catch((err) => {
            console.log(err);
        })
        ...
        <div>
            {isEmpty(blog) ? <h1 className='single-blog-loading'>{dataState}</h1> :
                <main className='main-single-blog'>
                    ...
                    <img onClick={editBlog} className='single-blog-edit' src='/assets/edit.png' alt='Edit Icon' />
                </main>}
        </div>
